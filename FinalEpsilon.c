...
if (normalizationType==1){ // powertype normalization
	for(i=0;i<=maxHistoryLengthFormula; i++){
		...
		//
			...
			if(info) {
				printf("hardware constraints applied for history length. Epsilons for longer history bigger than %d estimated by 1/2", maxHistoryLengthHardware);
			}
		}
	}
