...
//epsilons = calculateEpsilons(data, sourceLength, HistoryLength(sourceLength));//main calculation of frequencies and epsilons
    ...
    if( a->method_of_epsilon==1){
        finalEpsilon = FinalEpsilon(epsilons, HistoryLength(number_of_bytes), historyLength, 1,2);
        ...
        finalEpsilon =FinalEpsilon(epsilons,HistoryLength(number_of_bytes),historyLength,2,2);
		printf("Final epsilon based on squares %.15Lf\n",finalEpsilon);
    }
    else if(a->method_of_epsilon==3){
        finalEpsilon =FinalEpsilon(epsilons,HistoryLength(number_of_bytes),historyLength,2,1);
		printf("Final epsilon based on linear new method %.15Lf\n",finalEpsilon);
    
    }
	else if(a->method_of_epsilon==4){
        finalEpsilon =FinalEpsilon(epsilons,HistoryLength(number_of_bytes),historyLength,2,3);
